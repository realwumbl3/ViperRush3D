...
export function createInputController({
    ...
    function turnAxisFromPointerEvent(e) {
        const pad = document.getElementById('touch-left-pad');
        if (!pad) {
            const halfW = Math.max(1, window.innerWidth * 0.5);
            const raw = ((window.innerWidth * 0.5) - e.clientX) / halfW;
        ...
        const raw = (cx - e.clientX) / halfW;
        const scaled = Math.max(-1, Math.min(1, raw * touchTurnSensitivity));
        return Math.abs(scaled) < touchTurnDeadzone ? 0 : scaled;
    }

    function speedAxisFromPointerEvent(e) {
        const pad = document.getElementById('touch-right-pad');
        if (!pad) {
    ...
    function onPointerDown(e) {
        ...
        if (isMobileNow()) {
            ...
            const leftPad = document.getElementById('touch-left-pad');
            const rightPad = document.getElementById('touch-right-pad');
            const isTurnPad = !!(
                (e.target && typeof e.target.closest === 'function' && e.target.closest('#touch-left-pad')) ||
                (leftPad && leftPad.contains(e.target))
            );
            const isSpeedPad = !!(
                (e.target && typeof e.target.closest === 'function' && e.target.closest('#touch-right-pad')) ||
                (rightPad && rightPad.contains(e.target))
            );
            if (!isTurnPad && !isSpeedPad) return;
            if (isTurnPad) {
    ...
    function onTouchPadGesture(e) {
        ...
        if (!t.closest('#touch-left-pad') && !t.closest('#touch-right-pad')) return;
        if (e.cancelable) e.preventDefault();
    }
